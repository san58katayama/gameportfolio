...
const translations = {
  ja: {
    ...
    work1_details_summary: "こだわり＆得られた経験",
    work1_details_content: "JAMの制作なのですが1本のゲームとして完成させたく、JAMの作品とは思えないボリューミーなゲームに仕上げることができました<br>音楽はチームメンバーが一から制作しているのでこだわりがあります<br>テンキーで遊べて農業の仕様とマッチするようにこだわりました、ゲーム画面が見やすく音楽ゲーム✖農場ゲームといったやりたくなる面白さを大切にしました<br><br>ーーーーーーーーーーーーーーーーーーーーーーーーーーーーー<br><br>仕様書の書き方とUIのデザイン力そしてギミックでどう面白くなるかを考え、落とし込む能力を高めましたリズムマシンという特殊デバイスのゲーム制作になりますのでUnityとの接続の仕方や展示する時のアクシデントなどの臨機応変の土台も高めることが出来ました",
    ...
    work2_desc:      "「あなたは”白身”、黄身をこぼさず目的地までたどり着こう！」キッチンから飛び出し、森を抜け、最後にはなんと○○に進む！道中には、卵を狙うカラスなどたくさんの困難が待ち構える！協力してワイワイ！黄身がこぼれるドキドキ！協力2Dジャンプアクションゲーム！！",
    work2_details_summary: "こだわり＆得られた経験",
    work2_details_content: "白身を直感的に動かしたいと考え、マウスやボタンではなくタッチ操作で動かすというのを考え軸にしました<br>白身が左右で分かれているため2人でも1人でもできるゲーム性に出来ることができ、ハラハラとドキドキを味わえるよう上に上っていき卵の孵化を目指す仕様に落とし込みました<br>レベルデザインと仕様書を担当し、遊んでもらう人に遊びながら覚えるというのを意識しました<br><br>ーーーーーーーーーーーーーーーーーーーーーーーーーーー<br><br>仕様書のプログラマ、デザイナーに伝わる書き方<br>タスク振りわけ、会議進行などのプランナーの基礎的な仕事を経験することが出来ました<br>レベルデザインの書き方と面白さと狙いを伝える能力を高めることが出来ました<br>様々な学校の方がいらっしゃる場なので不安にさせない、嫌な気持ちにさせないコミュニケーション能力を大事にしもっと高めることが出来ました",
    ...
    work3_details_content: "unity1weekを個人製作で行いました、テーマがうらで他の人とは違う案を作りたく、スーパーマリオ オデッセイの2Dと3Dが切り替わる瞬間にインスピレーションを受け、表の世界と裏の世界を同時に動かす案が生れました。そしてお互い同じ重力だとただ単に動かすだけになってしまうので裏の世界の重力を反転させ、動かすだけで面白いゲームに仕上げることが出来ました<br><br>ーーーーーーーーーーーーーーーーーーーーーーーーーーー<br><br>初の個人制作なのでUnityの機能に当たる、タイルマップやエフェクト、そしてプログラム全般を学ぶ機会になりました<br>unity1weekが終了した後もブラッシュアップを行い、展示をさせていただき、お客様の声を聴いて更なるブラッシュアップを行える行動力と基礎的能力を高められました<br><br>これを活かし次の個人製作につなげられる土台を得ました",
    ...
    work4_details_content: "初のブラウザゲーム制作にあたり、分かりやすく、すぐに遊べる事を<br>意識しました！そのため簡単なゲームの中で生まれる感情を大切にし<br>スマホでもPCでも出来るようにしました！<br>デザイン面でも昔ながらのゲームセンターとキーボルダーゲームを<br>イメージしました！<br><br>ーーーーーーーーーーーーーーーーーーーーーーーーーーーーー<br><br>githubを使用しブラウザに投稿する方法を学びました！<br>ルールを簡単にし、ゲーム性を面白くすればしっかり遊べるゲームが制作できる事",
    ...
    work5_details_content: "学校内のゲームJAMに参加し制作した作品です<br>テーマは【発射】で考えられる案は沢山ありました<br>何かを発射するのはシンプルなので違った案にしたいと考え銃の反動で進む【反動✖イライラ棒ゲーム】を制作しました！シンプルなデザインですがエフェクトにこだわりを入れております<br>ゴールにたどり着くまでに死んで攻略そしてたどり着いた時のエフェクトと達成感の面白さを追求しました<br><br>ーーーーーーーーーーーーーーーーーーーーーーーーーーーーー<br><br>チームには後輩の子がいたので案をどう考えて面白さを出すかを一緒に考えながら育成＋後輩にどう教えるかのコミュニケーション能力を高めました<br><br>プログラマと寄り添い、いつでも相談をしてもらえる空気の良さを学び意識しました<br><br>タイルマップでのレベルデザインでルートは何種類もある！という意識の元違った楽しさ面白さをだせるレベルデザインを考えて実行する能力を高めました",
    ...
    recent_item1_summary: "得られたこと",
    recent_item1_details: "案をまとめて、メインプランナーのタスクや振り分けを<br>行いました！面白さや楽しさを設計することはもちろん<br>会議中も楽しく出来る会議を心がけました！",
  ...
  en: {
    ...
    work2_details_content: "I wanted players to move the egg white intuitively, so I made touch controls the core design instead of mouse or button inputs.<br>Since the egg white is divided into left and right parts, it can be played by either one or two players. To create excitement and anticipation, I designed the specification to climb upwards and aim for the egg to hatch.<br>I was in charge of level design and GDD (Game Design Document), focusing on letting players learn the mechanics naturally while playing.<br><br>--------------------------------------------------<br><br>I learned how to write specifications that clearly communicate ideas to programmers and designers.<br>I gained experience in basic planner tasks such as assigning tasks and facilitating meetings.<br>I improved my ability to write level design documentation and convey the fun and objectives of the design.<br>Since the team consisted of students from various schools, I prioritized communication skills to make everyone feel comfortable and valued, further strengthening my interpersonal skills.",
    ...
    work4_details_content: "For my first browser game, I focused on making it easy to understand and playable immediately!<br>I valued the emotions created within a simple game, making it accessible on both smartphones and PCs.<br>For the design, I imagined a traditional game arcade and a keychain game!<br><br>--------------------------------------------------<br><br>I learned how to use GitHub to publish games to the browser!<br>I realized that by keeping the rules simple and the gameplay fun, we can create a game that is truly engaging.",
    ...
    recent_item1_summary: "What I Gained",
    recent_item1_details: "I organized ideas, created tasks for the main planner, and distributed them!<br>Of course, I focused on designing fun gameplay, but I also made sure our meetings themselves were enjoyable!",
